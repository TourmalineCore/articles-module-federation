...
Uses the `@module-federation/vite` plugin. Navigation is static: nav items are defined in `layout/src/nav.ts` and bundled at build time.
...
Each service starts on its own port. Each one reads the URLs of its neighbors from `.env.local` — absolute addresses in the form `http://localhost:<port>`. Start layout first — hosts expect its `layout.js` (remote entry) to already be available. Open a separate terminal for each application and run:
