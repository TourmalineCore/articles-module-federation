...
Each service starts on its own port. Each one reads the URLs of its neighbors from `.env.local` — absolute addresses in the form `http://localhost:<port>`. Open a separate terminal for each application and run:
